...
import { TxtToPdfLoader } from "@/components/tools/document/DocumentNewLoaders";

export const metadata: Metadata = {
  title: "TXT → PDF Dönüştür — Ücretsiz Online Araç",
  description: "Metin dosyasını (.txt) PDF'e dönüştür. Satır kılavuzu, monospace font, otomatik sayfalama.",
  keywords: ["txt pdf", "metin pdf dönüştür", "text to pdf", "txt dönüştür"],
};

export default function TxtPdfPage() {
  return (
    <div className="min-h-screen py-10">
      <div className="container max-w-3xl">
        <nav className="flex items-center gap-2 text-xs mb-6" style={{ color: "var(--color-text-3)" }}>
          ...
          <span>/</span>
          <span style={{ color: "#3B82F6" }}>TXT → PDF</span>
        </nav>
        <div className="flex items-start gap-4 mb-8">
          ...
          <div>
            <h1 className="font-display text-2xl font-medium mb-1" style={{ color: "var(--color-text)" }}>TXT → PDF</h1>
            <p className="text-sm" style={{ color: "var(--color-text-2)" }}>Metin dosyasını Courier monospace font ile PDF&apos;e dönüştür. Otomatik sayfalama.</p>
          </div>
        </div>
        <div className="flex items-center justify-between px-4 py-2.5 rounded-lg border mb-6 text-xs" style={{ borderColor: "var(--color-border)", background: "var(--color-surface)" }}>
          <span style={{ color: "var(--color-text-2)" }}>Ücretsiz · Otomatik sayfa kılavuzu</span>
