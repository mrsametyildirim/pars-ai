...
export default function AudioTrimClient() {
  ...
  async function trim() {
    ...
    if (startSec === null || endSec === null) {
      setError("Geçerli bir zaman giriniz. Örnek: 00:01:30 veya 90");
      return;
    }
    if (endSec <= startSec) { setError("Bitiş zamanı başlangıçtan büyük olmalı."); return; }
    ...
    try {
      ...
      await ffmpeg.exec(["-i", inputName, "-ss", String(startSec), "-to", String(endSec), ...codecArgs, outputName]);
      ...
      a.href     = url;
      a.download = file.name.replace(/\.[^.]+$/, "") + "_trim" + (isWav ? ".wav" : ".mp3");
      a.click();
      ...
      {!file ? (
        ...
        <>
          ...
          <div style={{ display: "flex", gap: "1rem" }}>
            {[
              { label: "Başlangıç", value: startStr, onChange: setStart, hint: "örn: 00:00:30" },
              { label: "Bitiş",     value: endStr,   onChange: setEnd,   hint: "örn: 00:01:00" },
              ...
              <div key={label} style={{ flex: 1 }}>
                ...
                <input type="text" value={value} placeholder={hint} onChange={e => onChange(e.target.value)}
          ...
          <p style={{ color: textTer, fontSize: "0.73rem", marginTop: "-0.75rem" }}>
            Format: ss:dd:sn veya saniye (örn: 90 = 1:30). Çıktı: WAV girdi için WAV, diğerleri için MP3.
          </p>
