...
export default function AudioConcatClient() {
  ...
  async function concat() {
    if (files.length < 2) { setError("En az 2 ses dosyası eklemelisiniz."); return; }
    setLoading(true);
    setError("");
    setProgress(0);
    ...
    try {
      ...
      a.href     = url;
      a.download = "birlestir.mp3";
      a.click();
      URL.revokeObjectURL(url);

      for (let i = 0; i < files.length; i++) ffmpeg.deleteFile(`input_${i}.mp3`);
      ...
      setError("Birleştirme başarısız. Tüm dosyaların aynı formatta (MP3) olduğundan emin olun.");
      console.error(e);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      >
        ...
        <p style={{ color: textPri, fontWeight: 500, marginBottom: "0.25rem" }}>Ses dosyaları ekle</p>
        <p style={{ color: textTer, fontSize: "0.8rem" }}>Birden fazla dosya seçilebilir · MP3, WAV, M4A</p>
        <input ref={inputRef} type="file" accept=".mp3,.wav,.m4a,.aac,audio/*" multiple style={{ display: "none" }}
          ...
          {files.map((f, idx) => (
            <div key={idx} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.65rem 0.875rem", background: surface, borderRadius: "10px", border: `1px solid ${border}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: textTer, fontSize: "0.7rem" }}>{(f.size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
      ...
      <button onClick={concat} disabled={loading || files.length < 2} style={{
        ...
        {loading ? "Birleştiriliyor…" : `${files.length} Ses Dosyasını Birleştir`}
      </button>
    </div>
  );
}
