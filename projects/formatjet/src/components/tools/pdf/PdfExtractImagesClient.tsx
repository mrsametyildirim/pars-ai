...
export default function PdfExtractImagesClient() {
  ...
  async function extract() {
    ...
    try {
      ...
      for (let i = 1; i <= total; i++) {
        ...
        zip.file(`sayfa-${String(i).padStart(3, "0")}.jpg`, arr);
      }
  ...
  const downloadName = file ? file.name.replace(/\.pdf$/i, "-gorseller.zip") : "output.zip";

  const SCALES = [
    { value: 1, label: "Normal (72 dpi)" },
    { value: 2, label: "Yüksek (144 dpi)" },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      {!file ? (
        ...
        >
          ...
          <p style={{ color: textTer, fontSize: "0.8rem" }}>Her sayfa ayrı JPEG olarak ZIP içinde çıkar</p>
        ...
        <>
          ...
          <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: "12px", padding: "1rem" }}>
            <label style={{ color: textSec, fontSize: "0.8rem", display: "block", marginBottom: "0.5rem" }}>Çözünürlük</label>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {SCALES.map(s => (
                <button key={s.value} onClick={() => setScale(s.value)} style={{
                  flex: 1, padding: "0.45rem", borderRadius: "8px", border: `1px solid ${scale === s.value ? accentColor : border}`,
                  background: scale === s.value ? `${accentColor}1A` : "transparent",
                  color: scale === s.value ? accentColor : textSec, cursor: "pointer", fontSize: "0.75rem", fontFamily: "inherit",
                }}>
                  {s.label}
                </button>
              ))}
          ...
          {resultUrl && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.875rem 1rem", background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: "12px" }}>
              <p style={{ color: "#10B981", fontWeight: 600, fontSize: "0.9rem" }}>{pageCount} sayfa görsel ZIP'e paketlendi</p>
          ...
          <button onClick={extract} disabled={loading} style={{
            ...
            {loading ? "İşleniyor…" : "Görselleri Çıkar"}
          </button>
        </>
      )}
    </div>
